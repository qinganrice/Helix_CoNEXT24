...
def choose_action(user_set, H, rt_history): # t is Frame #
    # print("Entering......")
    ...
    cap_per_ue = np.zeros((actions_num,P_N_UE))
    # print('Current Frame is:',t)
    for action in range (0, actions_num):
        
        ue_select, idx = sel_ue(user_set, action)
        ...
        for count, ue in enumerate (ue_select):
            pf[action] += data_rate[count] / rt_history[ue]

        # cap_total[action], _, cap_per_ue[action,0:idx] = data_process(np.reshape(H_s,(BS_ANT,-1)),idx,mod_select)

        
    
    return np.argmax(pf), cap_total[np.argmax(pf)], cap_per_ue[np.argmax(pf),:]
