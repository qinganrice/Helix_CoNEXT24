...
for tti in range (0, 10):
    ...
    # achieved_data = np.zeros((3,))
    # [Num_UE, Num_BS, Num_tti, Num_RBG]
    # start = time.time()
    for rbg in range (0, Num_RBG):
        ...
        for sl in range (0, Num_slice):
            ...
            # print(cap_rbg_slice[tti,rbg,sl])


    # sorted_indices_2d = np.zeros((Num_slice,2,Num_RBG))

    # for i in range (Num_slice):
    #     sorted_indices_2d[i,:,:] = sort_vector(cap_rbg_slice[tti,:,i])

        
    # sorted_indices_2d = sorted_indices_2d.tolist()
    cap_rb_sl = cap_rbg_slice[tti,:,:]
    inter_slice = time.time()
    while any(element > 0 for element in delta_data) and (cap_rb_sl.size):
        ...
        #     new_vector = []
        #     for j, num in enumerate (sorted_indices_2d[s][1]):
        #         if num == rbg_num:
        #             index = j
        #         else:
        #             new_vector.append(num)
        #     sorted_indices_2d[s][1] = new_vector
        #     sorted_indices_2d[s][0].pop(index)
