...
// #include "func.h"
#include <chrono>
#include "alloc_rb_orth.h"

using namespace H5;

// If any positive element in the list
...
// ******************************************** CONFIGURE = 400 *******************************************************************
    ...
    std::cout << "READING CHANNEL GOOD FOR NOW" << std::endl;
    ...
    {   
        ...
        while (any_positive(delta_slice) && !RBG_remain.empty()) {
            ...
            if (parallel == 1)
            {
                ...
                {
                    sel_sl = 4;
                }else if (ue_index < 118)
                {
                    sel_sl = 5;
                }else if (ue_index < 163)
                {
                    sel_sl = 6;
                }else if (ue_index < 200)
                {
                    sel_sl = 7;
                }   
                ...
                // arma::vec cg_sel_sl(sl_ue_list.size(), arma::fill::zeros);

                // for (size_t i=0; i<sl_ue_list.size(); ++i) {
                //     cg_sel_sl[i] = cg_rb_ue(rbg_index,sl_ue_list[i]);
                // }
                ...
                // std::cout << "Before Rate" << std::endl;
                auto[sel_UE_list, sum_of_rate] = alloc_rb(group_target, SEL_UE, Num_UE_ps, sel_sl, sel_rbg, tti, H_t, ue_index, sl_ue_list,cg_ue_vec);
                std::cout << "Get Rate" << std::endl;
